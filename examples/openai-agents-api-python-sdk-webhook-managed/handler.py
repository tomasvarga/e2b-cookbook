...
# Worker image with codex exec-server baked in (see build_template.py). The
# executor launch below still installs codex if missing, so the default E2B
# image works too, just slower on the first turn.
WORKER_TEMPLATE = os.environ.get("E2B_WORKER_TEMPLATE", "openai-agents-api-python-sdk-webhook-managed")
...
async def reconcile(session_id: str) -> None:
    ...
    if (
        ...
        return
    action = next(
        (a for a in session["required_actions"] if a["type"] == "environment_connection"), None
    )
    if session["status"] != "failed" and action is None:
        return
    sandbox_id = await find_sandbox(session_id)
    if session["status"] == "failed":
        # Pause instead of kill so a failed session's worker can be inspected
        # later (files and memory kept). Returns False if already paused.
        if sandbox_id is not None:
            paused = await AsyncSandbox.pause(sandbox_id)
            print(
                json.dumps(
                    {"session_id": session_id, "sandbox_id": sandbox_id, "action": "paused" if paused else "already_paused"}
                ),
                flush=True,
            )
        return
    ...
    print(
        json.dumps(
            {"session_id": session_id, "sandbox_id": sandbox.sandbox_id, "action": "started"}
        ),
        flush=True,
    )
...
async def drain_queue(db: sqlite3.Connection) -> None:
    while True:
        row = db.execute(
            "SELECT session_id, attempts FROM jobs WHERE retry_at <= ? LIMIT 1", (time.time(),)
        ).fetchone()
        if row is None:
...
async def webhook(request: Request) -> JSONResponse:
    ...
    if event["type"] == "agent.session.failed" or (
        ...
        print(
            json.dumps(
                {"event_id": event["id"], "session_id": event["data"]["id"], "action": "enqueued"}
            ),
            flush=True,
        )
