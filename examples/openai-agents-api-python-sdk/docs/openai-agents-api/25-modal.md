...
slug: modal
order: 25
source: https://preview-docs-git-agents-alpha-openai.vercel.app/alphas/agents-api/modal
---
...
- **[Application-managed](#before-you-begin):** Follow this guide to start and stop sandboxes from your application, or run the [SDK example](https://github.com/OpenAI-Early-Access/agents-api-python-preview/tree/main/examples/self_hosted_sandbox/application_managed/modal).
- **[Webhook-managed](https://github.com/OpenAI-Early-Access/agents-api-python-preview/tree/main/examples/self_hosted_sandbox/webhook_managed/modal):** Deploy a handler that starts or reconnects sandboxes from OpenAI webhooks.

See [Manage Sandbox Lifecycle](./05-manage-sandbox-lifecycle.md) to compare the two modes.

## Before you begin

You need the `agent_api_sdk` Python SDK, an OpenAI project API key, a Modal token ID and secret, and the Codex CLI package.
...
```bash
pip install "git+https://github.com/OpenAI-Early-Access/agents-api-python-preview.git" modal
```

## 1\. Set up the Modal environment

Save this code as `modal_setup.py`. It defines an image with the Codex CLI package and starts the Modal sandbox.
...
async def start_sandbox(
    ...
    client = await modal.Client.from_credentials.aio(modal_token_id, modal_token_secret)
    app = await modal.App.lookup.aio(
        "agents-api-modal-guide",
    ...
    return await modal.Sandbox.create.aio(
        ...
        image=(
            ...
            .run_commands(
                ...
                "mkdir -p /workspace",
                "echo 'Migrate a synchronous Python service to an async API while preserving behavior.' > /workspace/brief.txt",
            )
            .workdir("/workspace")
        ),
        secrets=[modal.Secret.from_dict({"CODEX_API_KEY": executor_api_key})],
        workdir="/workspace",
        timeout=300,
        client=client,
    )
...
This code creates the session with your agent configuration, connects the Modal sandbox, runs the task, and streams the result.

```python
import asyncio
import os
from agent_api_sdk import AgentAPISDK
from modal_setup import start_sandbox
    ...
    async with AgentAPISDK(api_key=application_api_key) as client:
        try:
            ...
            events = session.stream(
                input=(
                    "Read /workspace/brief.txt and produce a five-step migration plan."
                )
            )
            try:
...
- [Modal Python SDK reference](https://modal.com/docs/sdk/py/latest/Sandbox)
- [Modal JavaScript/TypeScript SDK reference](https://modal.com/docs/sdk/js/latest/Sandbox)
